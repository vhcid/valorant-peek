...
import {
  MessageActionRow, MessageButton, Permissions, Util,
} from 'discord.js';
...
// kontol lu node fetch ngentot ajg bajingan
export const fetch = (url, options = {}) => new Promise((resolve) => {
  const req = https.request(url, {
    ...
    let chunks = [];
    resp.on('data', (chunk) => chunk.push(chunk));
    resp.on('end', () => {
...
// file utils - Implementations Of Forked API

export const asyncReadFile = (file) => new Promise(((resolve, reject) => {
...
export const asyncReadJSONFile = async (path) => JSON.parse((await asyncReadFile(path)).toString());
...
export const removeAlertButton = (id, uuid) => new MessageButton().setCustomId(`removealert/${uuid}/${id}/${Math.round(Math.random() * 10000)}`).setStyle('DANGER').setLabel('Remove Alert')
  .setEmoji('✖');
...
export const { escapeMarkdown } = Util;
